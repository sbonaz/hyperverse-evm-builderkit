// USING MOCHA FRAMEWORK FOR TESTING + CHAI lybrary
...
describe("BYOM", function () {
  ...
  beforeEach(async () => {
    ...
    byomFactoryCtr = await BYOMFactory.deploy(
      byomMainCtr.address,
      owner.address
    );
    await byomFactoryCtr.deployed();
    await byomFactoryCtr
      .connect(alice)
      .createInstance(alice.address, "BYOM", "BYM", "EUR", "1", "6");
    aliceProxyContract = await BYOMMain.attach(
      await byomFactoryCtr.getProxy(alice.address)
    );
  });
  ...
  describe("Initial State Variables", function () {
    it("Master Contract should match byomMainCtr", async function () {
      expect(await byomFactoryCtr.masterContract()).to.equal(
        byomMainCtr.address
      );
    });
    ...
    it("Should have correct balance of byoms using balance() and balanceOf()", async function () {
      expect(await aliceProxyContract.connect(alice).balance()).to.equal(
        initialSupply
      );
      expect(await aliceProxyContract.balanceOf(alice.address)).to.equal(
        initialSupply
      );
    });
  });
  ...
  describe("Transfer", function () {
    ...
    it("Should approve funds tranfer using approve() and check spend amount using allowance()", async function () {
      ...
      const allowance = await aliceProxyContract.allowance(
        sourceAccount,
        targetAccount
      );
    ...
    it("Should transfer allowance funds between accounts using tranferFrom()", async function () {
      ...
      await aliceProxyContract
        .connect(alice)
        .approve(authorizedAccount, amount);
